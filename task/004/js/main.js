        ...
        .done((result) => {
            ...
            for (const person of resultArray) {
                ...
                const $DIV = $(`<form id="f${person.no}"></form>`)
                    .addClass("person")
                    .appendTo($LI);
                const $LABEL = $(`<label for="input${person.no}"></label>`).appendTo($DIV);
                const $IMG = $(`<img>`)
                    ...
                    .appendTo($LABEL);
                const $INFO = $(`<div></div>`).addClass("person_info").appendTo($DIV);
                ...
                $(`<input>`)
                    ...
                    .appendTo($INFO);
                const $BTN = $(`<div></div>`).addClass("info_btnarea").appendTo($INFO);
                $(`<button>수정</button>`)
                    .addClass("info_edit")
                ...
                $(`<button>삭제</button>`)
                    ...
                    .attr("onclick", `deleteContact(${person.no}); return false;`)
                    .appendTo($BTN);
            }
...
function pagination(pageno, pagesize, total) {
    ...
    for (let i = pageno-2; i < pageno+pageBlock; i++) {
        if (i == pageno) {
            $page.append(`<li><a href="?pageno=${i}" class="now">${i}</a></li>`);
        ...
        if (i > totalPage) {
            ...
            $page.prepend(`<li><a href="?pageno=${current-1}">${current-1}</a></li>`);
        }
    }
    ...
    // 이전 버튼
    if (pageno > pageBlock) {
        $page.prepend(`<li><a href="?pageno=${pageno - pageBlock}">&lt&lt</a></li>`);
    }

    // 다음 버튼
    if (pageno <= totalPage - pageBlock) {
        $page.append(`<li><a href="?pageno=${pageno + pageBlock}">&gt&gt</a></li>`);
    }
}